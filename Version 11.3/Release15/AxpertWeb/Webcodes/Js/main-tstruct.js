...
$j(document).ready(function () {
    ...
    if (typeof AutosaveDraft != "undefined" && AutosaveDraft == "true") {
        if (_savedraftKeytime != "" && savedraftKeyCreatedtime == "") {
            savedraftKeyCreatedtime = _savedraftKeytime;
            _savedraftKeytime = "";
        }
        //if (savedraftKeyCreatedtime != "") {
    ...
    if (result.toLowerCase().indexOf("access violation") === -1) {
        ...
        if (tablehtml != "" && tablehtml == "<label>No data found.</label>") {
            ...
            if (resSplit.length > 1 && resSplit[1].toString() != "") {
                ...
                var buttonResizeHTML = `<button type="button" class="btn btn-light shadow-sm" onclick="saveFillgridColResize('` + fillGridDc + `','` + _fgCaption + `','` + AxActivefillGridName + `');">Save Resize</button>`;
...
function totalFillGridColVal(obj, _thisFgInd) {
    if (typeof FillGridTotalFldVal != "undefined" && FillGridTotalFldVal.length > 0 && FillGridTotalFldVal[_thisFgInd] != "") {
        ...
        _matchinflds.forEach(function (val, ind) {
            _inputString = _inputString.replace(":tot_" + val, columnSums[ind]);
        });
...
function DropzoneInit(dvId) {
    ...
    _thisDiv.each(function () {
        ...
        myDropzone.on("addedfile", function (file) {
            ...
            this.options.url = url + `TstFileUpload.ashx?thisFld=${$(id).attr("id").substr(9)}&filePath=${_thisFilePath}&dcNo=${GetFieldsDcNo($(id).attr("id"))}&attFldName=${GetFieldsName($(id).attr("id").substr(9))}&fileExt=${_uploadFileTypes}`;
        ...
        ASB.WebService.GetAutoIntelliFromAPI(tstDataId, ChangedFields, ChangedFieldDbRowNo, ChangedFieldValues, DeletedDCRows, _fname, "", "", _resTstHtmlLS, (res) => {
            ...
            try {
                ...
                setTimeout((viewele = matchInfo.autocomplete.view.element.$) => {
                    if (viewele.getBoundingClientRect().y + viewele.getBoundingClientRect().height > document.getElementsByClassName("tstructMainBottomFooter")[0].getBoundingClientRect().y + document.getElementsByClassName("tab-content")[0].getBoundingClientRect().y && transid != "a__fn") {
...
function ExcelImportGridCall(xmlOutput, xlDcNo) {
    ...
    if (data != "<GridList></GridList>") {
        try {
            ...
            GetProcessTime();
            ASB.WebService.GetExcelImportValues(ChangedFields, ChangedFieldDbRowNo, ChangedFieldValues, DeletedDCRows, xlDcNo, data, tstDataId, resTstHtmlLS, SuccGetResultValue, OnException);
        } catch (ex) {
            AxWaitCursor(false);
            ShowDimmer(false);
            if (dialogObj) {
