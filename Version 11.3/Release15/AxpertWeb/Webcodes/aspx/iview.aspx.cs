    ...
    public XmlDocument GetRedisData()
    {
        ...
        try
        {
            ...
            if (isRedisConnected)
            {
                ...
                ArrayList redisvaluesAnalyticsMetaData = fdrObj.GetPrefixedKeys("AX", false);
                ...
                if (redisvaluesAnalyticsMetaData.Count > 0)
                {
                    string keysData1 = String.Join("&amp;", (string[])redisvaluesAnalyticsMetaData.ToArray(Type.GetType("System.String")));
                    aaa.AppendLine(string.Format("<row FTYPE=\"{0}\" TNAME=\"{1}\"  CAPTION=\"{2}\" KEYSIZE=\"{3}\" TOTALKEYS=\"{4}\" KEYS=\"{5}\" /> ", "Analytics/Entity Metadata", "Analytics/Entity Metadata", "Analytics/Entity Metadata", "", redisvaluesAnalyticsMetaData.Count, keysData1));
                }
                elem.InnerXml = aaa.ToString();
            }
