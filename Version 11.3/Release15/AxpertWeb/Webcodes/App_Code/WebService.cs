...
namespace ASB
{
    ...
    public class WebService : System.Web.Services.WebService
    {
                ...
                if (ddlFldSqlParams == "1")
                {
                    ...
                    else
                    {
                        ...
                        if (ddlSqlPNames == "")
                        {
                            ...
                            {
                                iXml += tstData.GetFieldSqlParamsVars(fldName, _fldParamsfromRD, tstData.fieldValueXml, HttpContext.Current.Session["axGlobalVars"].ToString(), HttpContext.Current.Session["axUserVars"].ToString(), int.Parse(activeRow), parentFlds, tblSourceParams);
                                parentFlds = iXml.Split('♠')[1];
                                parentFldNames = iXml.Split('♠')[2];
                                iXml = iXml.Split('♠')[0];
                            }
                        ...
                        {
                            iXml += tstData.GetFieldSqlParamsVars(fldName, ddlSqlPNames, tstData.fieldValueXml, HttpContext.Current.Session["axGlobalVars"].ToString(), HttpContext.Current.Session["axUserVars"].ToString(), int.Parse(activeRow), parentFlds, tblSourceParams);
                            parentFlds = iXml.Split('♠')[1];
                            parentFldNames = iXml.Split('♠')[2];
                            iXml = iXml.Split('♠')[0];
                        }
        ...
        [WebMethod(EnableSession = true)]
        public string GetExcelImportValues(ArrayList fldArray, ArrayList fldDbRowNo, ArrayList fldValueArray, ArrayList fldDeletedArray, string frameNo, string s, string key, string resTstHtmlLS)
        {
            string requestProcess_logtime = string.Empty;
            ObjExecTr.SetCurrentTime();
            DateTime sTime1 = DateTime.Now;
            ...
            string result = string.Empty;
            DateTime eTime1 = DateTime.Now;
            result = tstData.CallGetExcelImportValuesWS(tstData, fldArray, fldDbRowNo, fldValueArray, fldDeletedArray, frameNo, Ixml);
            requestProcess_logtime += result.Split('♠')[0];
            result = result.Split('♠')[1];
            DateTime sTime2 = DateTime.Now;
            if (result != "")
