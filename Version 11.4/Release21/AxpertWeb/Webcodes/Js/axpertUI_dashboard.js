; (function ($, win) {
    //class axpertUI_dashboard {
    $_this = this;
    function axpertUI_dashboard() {
        options = {
            loader: {
                div: '.page-loader',
        ...
        */
        dashboardcardsPage = {
            activate(options) {
                ...
                try {
                    var ifURL = $("#middle1", parent.document);
                    if (ifURL.attr('src').indexOf('&edit=t') > -1)
                        setTimeout(function () { _this.editSaveCardDesignToggle(); }, 0);
                } catch (ex) { }
            ...
            _refreshData(axp_cardsid, singleLoad = false) {
                ...
                let result = "";
                $.ajax({
                    url: 'mainnew.aspx/GetCardsWithDataWS',
                    type: 'POST',
                    cache: false,
                    async: false,
                    data: JSON.stringify({
                    ...
                    success: function (data) {
                        if (data && data.d) {
                            try {
                                if (data.d !== "" && data.d.indexOf("SESSION_TIMEOUT") > -1) {
                                    window.location.href = "../aspx/sess.aspx";
                                    return;
                                }
                ...
                if (!reset && isEdit) {
                    ...
                    if (!reset && _this.options.designChanged) {
                        ...
                        if ((saveMsg = _this._saveCardDesign()) == "true") {
                            ...
                            //_this.activate();
                            parent.LoadIframe('processflow.aspx?dashboard=t');
                        } else {
                            try {
            ...
            _saveCardDesign() {
                ...
                let result = "";
                $.ajax({
                    url: 'processflow.aspx/saveCardsDesign',
                    type: 'POST',
                    cache: false,
                    async: false,
                    data: JSON.stringify({
            ...
            _drawCardsLayout() {
                ...
                $.axpertUI_dashboard.dashboardGrouping.init();
                $.axpertUI_dashboard.dashboardGrouping.groupingVars.cardsJson = _this.options.cards;
                if ($.axpertUI_dashboard.dashboardGrouping.groupingVars.groupJson && $.axpertUI_dashboard.dashboardGrouping.groupingVars.groupJson.length > 0) {
                    $.axpertUI_dashboard.dashboardGrouping.createGroupAccordions($(_this.options.staging.cardsFrame.cardsDiv));
                }


                _this.options.cards.forEach((card, index) => {
                    let cardType = card["cardtype"];
                    if (cardType?.toLowerCase() == "custom plugin") {
                        cardType = "html";
                        card["cardtype"] = "html";
                    }
                    ...
                    if (cardType) {
                        ...
                        var groupName = $.axpertUI_dashboard.dashboardGrouping.getGroupNameByCardId(cardId) || "";

                        if (groupName != "") {
                            $(`${_this.options.staging.cardsFrame.cardsDiv} .group-container[data-groupname="${groupName}"]`).append(cardElement);

                            $(`[data-groupname="${groupName}"]`).closest(".group-accordion").removeClass("d-none");
                        }
                        else {
            ...
            _getCardData(card) {
                ...
                if (card["cardtype"] == "html") {
                    card["html_editor_card"] = Handlebars.compile(card["html_editor_card"])(card);

                    if (!_this.cardsData) {
                        _this.cardsData = {};
                    }

                    _this.cardsData[card.cardname] = cardData;
                }
            ...
            _setOptionsCardInfo(cardElement, cardData, card) {
                ...
                if (card?.cardbuttons?.btnoption?.length > 0) {
                    ...
                    dropdownOptions.replaceWith($(`${btnoption.map((btn, btnInd) => `
                            ${btn?.Button ? `
                            <a href="javascript:void(0);" onclick="$.axpertUI_dashboard.dashboardcardsPage._callOptionsCardClick('${card.axp_cardsid}', '${btn.Button}', this);" class="btn btn-sm btn-light-primary btn-active-primary my-1 me-2">
            ...
            _callOptionsCardClick(cardId, btnName, clickElem) {
                ...
                let card = dashboardcardsPage.options.cards.filter(card => card.axp_cardsid == cardId)?.[0];
            ...
            _setCalendar(cardElement, cardData, card) {
                ...
                let headerOptions = [
                    {
                        ...
                        caption: "Month View",
                        onclick: `$.axpertUI_dashboard.dashboardcardsPage.options.cards[${cardElement.data("cardIndex")}].obj.changeView('dayGridMonth');`
                    },
                    {
                        icon: "calendar_view_week",
                        href: "javascript:void(0);",
                        caption: "Week View",
                        onclick: `$.axpertUI_dashboard.dashboardcardsPage.options.cards[${cardElement.data("cardIndex")}].obj.changeView('dayGridWeek');`
                    },
                    {
                        icon: "calendar_view_day",
                        href: "javascript:void(0);",
                        caption: "Day View",
                        onclick: `$.axpertUI_dashboard.dashboardcardsPage.options.cards[${cardElement.data("cardIndex")}].obj.changeView('timeGridDay');`
                    },
                    {
                        icon: "add",
                        href: "javascript:void(0);",
                        caption: "Add Event",
                        onclick: `$.axpertUI_dashboard.dashboardcardsPage._calendarEventClick('', ${cardElement.data("cardIndex")});`
                    }
                ];
                ...
                setTimeout(() => {
                    let _this = $.axpertUI_dashboard.dashboardcardsPage;
            ...
            _appendHTMLCardData(cardElement, card, shadowDomEle, objCardHTML, cardHTML) {
                ...
                if (objCardHTML.data("shadowDom") === false || objCardHTML.data("shadowdom") === false) {
            ...
            _registerPlugins() {
                ...
                $(`${_this.options.staging.cardsFrame.cardsDesigner} li input:checkbox`).off("change.showHideChkBox").on("change.showHideChkBox", () => {
                    _this.options.designChanged = true;
                });
        ...
        dashboardGrouping = {
            groupingVars: {
                ...
                rowCount: 0,
                groupAccordionHTML:
                    `<div class="col-xl-12 col-lg-12 col-sm-12 Invoice-content-wrap group-accordion d-none">
            ...
            init() {
                let _this = $.axpertUI_dashboard.dashboardGrouping;

                const payload = {
                    page: "DASHBOARD",
                    transId: "",
                    propertiesList: ["CARD_GROUP"]
                };

                _entityCommon.getAnalyticsDataSyncWS(payload, (data) => {
                    var cardGroup = JSON.parse(data).result.data.CARD_GROUP;
                    if (_entityCommon.isValid(cardGroup)) {
                        _this.groupingVars.groupJson = JSON.parse(cardGroup);
                    }
                }, (error) => {
                    showAlertDialog("error", "Error in grouping fetch" + error);
                })
            },

            getOtherSelectedCards(currentSelect) {
                let _this = $.axpertUI_dashboard.dashboardGrouping;
                const selectedCards = new Set();
                document.querySelectorAll('.card-select').forEach(select => {
                    if (select !== currentSelect) {
                        const values = $(select).val() || [];
                        values.forEach(value => selectedCards.add(parseInt(value)));
                    }
                });
            ...
            initializeSelect2(select) {
                let _this = $.axpertUI_dashboard.dashboardGrouping;
                const otherSelectedCards = _this.getOtherSelectedCards(select);
                const currentSelection = $(select).val() || [];

                const availableCards = _this.groupingVars.cardsJson.filter(card =>
                    !otherSelectedCards.has(card.axp_cardsid) || currentSelection.includes(card.axp_cardsid.toString())
                );
                ...
                $(select).select2({
                    ...
                    _this.updateAllDropdowns();
                    _this.validateField(this, 'cards');
                });
            },
            ...
            updateAllDropdowns() {
                ...
                document.querySelectorAll('.card-select').forEach(select => {
                    ...
                    _this.initializeSelect2(select);
                    $(select).val(currentVal).trigger('change.select2');
                });
            },

            addCardGroupingRow(existingData = null) {
                let _this = $.axpertUI_dashboard.dashboardGrouping;
                _this.groupingVars.rowCount++;
                const row = document.createElement('div');
                row.className = 'grid-row';
                row.innerHTML = `
            <div>${_this.groupingVars.rowCount}</div>
            <div>
                <input type="text" class="group-name" placeholder="Enter group name" value="${existingData ? existingData.groupname : ''}" onblur="$.axpertUI_dashboard.dashboardGrouping.validateField(this, 'groupname')">
            ...
            <div>
                <input type="number" class="order-no" value="${existingData ? existingData.orderno : _this.groupingVars.rowCount}" onblur="$.axpertUI_dashboard.dashboardGrouping.validateField(this, 'orderno')">
        ...
        `;
                ...
                document.getElementById('grid-body').appendChild(row);

                const select = row.querySelector('.card-select');
                _this.initializeSelect2(select);

                if (existingData && existingData.cards) {
                    $(select).val(existingData.cards).trigger('change');
                }

                // Add input event listeners
                row.querySelector('.group-name').addEventListener('input', function () {
                    _this.validateField(this, 'groupname');
                });

                row.querySelector('.order-no').addEventListener('input', function () {
                    _this.validateField(this, 'orderno');
                });
            },
            ...
            validateField(element, fieldType) {
                ...
                switch (fieldType) {
                    case 'groupname':
                        ...
                        break;
                    case 'cards':
                        isValid = $(element).val() && $(element).val().length > 0;
                        break;
                    case 'orderno':
                        isValid = element.value.trim() !== '' && !isNaN(element.value);
                        break;
                }
            ...
            validateRow(row) {
                let _this = $.axpertUI_dashboard.dashboardGrouping;
                const groupName = _this.validateField(row.querySelector('.group-name'), 'groupname');
                const cards = _this.validateField(row.querySelector('.card-select'), 'cards');
                const orderNo = _this.validateField(row.querySelector('.order-no'), 'orderno');

                return groupName && cards && orderNo;
            },
            ...
            updateSerialNumbers() {
                ...
                _this.groupingVars.rowCount = rows.length;
            },
            ...
            submitCardGrouping() {
                ...
                const output = Array.from(rows)
                    .map(row => ({
                        groupname: row.querySelector('.group-name').value,
                        cards: $(row.querySelector('.card-select')).val().map(Number),
                        orderno: parseInt(row.querySelector('.order-no').value)
                    }))
                ...
                var groupJson = {
                    page: "DASHBOARD",
                    transId: "",
                    properties: { CARD_GROUP: JSON.stringify(output) },
                    allUsers: true,
                    confirmNeeded: true
                };

                _entityCommon.setAnalyticsDataWS(groupJson,
                    () => {
                        showAlertDialog("success", "Card Groups saved successfully.");
                        window.location.reload();
                    },
                    (error) => {
                        showAlertDialog("error", "Error saving grouped data:", error);
                    }
                );
            },
            ...
            checkDuplicateCards() {
                let _this = $.axpertUI_dashboard.dashboardGrouping;
                const rows = document.querySelectorAll('.grid-row');
                const cardMap = new Map(); // Map to store card -> groups mapping
                let duplicates = [];

                // Reset previous errors
                rows.forEach(row => row.classList.remove('has-error'));

                rows.forEach(row => {
                    const groupName = row.querySelector('.group-name').value;
                    const selectedCards = $(row.querySelector('.card-select')).val() || [];

                    selectedCards.forEach(cardId => {
                        if (!cardMap.has(cardId)) {
                            cardMap.set(cardId, [groupName]);
                        } else {
                            cardMap.get(cardId).push(groupName);
                        }
                    });
                });

                // Check for duplicates
                cardMap.forEach((groups, cardId) => {
                    if (groups.length > 1) {
                        const cardName = _this.groupingVars.cardsJson.find(c => c.axp_cardsid === parseInt(cardId)).cardname;
                        duplicates.push({
                            card: cardName,
                            groups: groups
                        });
                ...
                if (duplicates.length > 0) {
                    const errorMessages = duplicates.map(d =>
                        `Card "${d.card}" is mapped to multiple groups: ${d.groups.join(', ')}`
                    );

                    showAlertDialog("error", errorMessages);

                    // Highlight rows with duplicate cards
                    rows.forEach(row => {
                        const selectedCards = $(row.querySelector('.card-select')).val() || [];
                        if (selectedCards.some(cardId =>
                            duplicates.some(d => d.card === _this.groupingVars.cardsJson.find(c => c.axp_cardsid === parseInt(cardId)).cardname)
                        )) {
                            row.classList.add('has-error');
                            row.querySelector('.card-select').classList.add('error');
                        }
                    });
            ...
            validateAndSubmit() {
                ...
                rows.forEach(row => {
                    if (!_this.validateRow(row)) {
                        isValid = false;
                    }
                });

                // Then check for duplicate cards
                if (isValid) {
                    isValid = _this.checkDuplicateCards();
                }

                if (isValid) {
                    _this.submitCardGrouping();
                } else if (!document.getElementById('duplicate-error').style.display) {
                    showAlertDialog('Error', 'Please fill in all required fields correctly.');
                }
            },

            getGroupNameByCardId(cardId) {
                let _this = $.axpertUI_dashboard.dashboardGrouping;
                if (_this.groupingVars.groupJson && _this.groupingVars.groupJson.length > 0) {
                    for (const group of _this.groupingVars.groupJson) {
            ...
            createGroupAccordions(container) {
                let _this = $.axpertUI_dashboard.dashboardGrouping;
                var html = ``;
                if (_this.groupingVars.groupJson.length > 0) {
                    _this.groupingVars.groupJson.forEach((group, index) => {
                        group.index = index;
                        html += Handlebars.compile(_this.groupingVars.groupAccordionHTML)(group);
                    });

                    html += Handlebars.compile(_this.groupingVars.groupAccordionHTML)({ index: _this.groupingVars.groupJson.length, groupname: "Others" });
                }
            ...
            resetCardGrouping() {
                ...
                _entityCommon.setAnalyticsDataWS(groupJson,
                    () => {
                        showAlertDialog("success", "Card Groups resetted successfully.");
                        window.location.reload();
                    },
                    (error) => {
                        showAlertDialog("error", "Error saving grouped data:", error);
            ...
            openCardGrouping() {
                ...
                if (!$('#groupingModal').hasClass("content-loaded")) {
                    if (_this.groupingVars.groupJson.length > 0) {
                        _this.groupingVars.groupJson.forEach(group => {
                            _this.addCardGroupingRow(group);
                        });
                    }
                    else {
                        _this.addCardGroupingRow();
                    }
                    $('#groupingModal').addClass("content-loaded")
                }
            }
        }
        ...
        */
        ...
        init = function (initOptions) {
            ...
            dashboardcardsPage.activate(this.options.dashboardcardsPage);
            return this.options;
        }
        //==========================================================================================================================
        return { options, dashboardcardsPage, browser, init, dashboardGrouping }
    }
    $.axpertUI_dashboard = new axpertUI_dashboard();
})(jQuery, window);
