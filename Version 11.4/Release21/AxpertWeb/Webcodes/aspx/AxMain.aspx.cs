...
public partial class AxMain : System.Web.UI.Page
{
    ...
    private string CheckListViewAsDefault(string structName)
    {
        try
        {
            ...
            var match = jsonArray
                .FirstOrDefault(x => string.Equals((string)x["stype"], "Tstruct", StringComparison.OrdinalIgnoreCase) && string.Equals((string)x["structname"], structName, StringComparison.OrdinalIgnoreCase));
            if (match != null)
                return (string)match["propsval"];
            var allForms = jsonArray
                .FirstOrDefault(x => string.Equals((string)x["stype"], "Tstruct", StringComparison.OrdinalIgnoreCase) && string.Equals((string)x["structname"], "ALL Forms", StringComparison.OrdinalIgnoreCase));

            if (allForms != null)
                return (string)allForms["propsval"];
        }
        catch (Exception ex)
        {
        }
