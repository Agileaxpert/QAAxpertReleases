...
function selectGetsearch(viewName) {
    ...
    let el;
    if (viewName.toLowerCase() == "active list")
        el = $('a.menu-link[title="' + viewName + '"]').closest('.menu-item')[0];
    else
        el = $('a[onclick*="' + viewName + '"]').closest('.menu-item')[0];

    if (el) {
        el.classList.add('hover');
        el.setAttribute('tabindex', '-1');
        // Expand all parent .menu-sub if any
        $(el).parents('.menu-sub').each(function () {
            $(this).show();
            $(this).prev('.menu-link').addClass('show');
        });

        el.focus({ preventScroll: true });
        el.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
    // Remove 'hover' class on focusout
