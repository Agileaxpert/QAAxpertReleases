...
var pageNo = 1;
var rowsCountHelper;
let tableScrollResizeBound = false;
const FRONTEND_STREAM_CHUNK_SIZE = 100;
const FRONTEND_STREAM_WINDOW_ROWS = 200;
...
class Entity {
    constructor() {
        ...
        this.streamingAllRecords = false;
        this.allRowsJson = [];
        this.frontendChunkSize = FRONTEND_STREAM_CHUNK_SIZE;
        this.frontendWindowMaxRows = FRONTEND_STREAM_WINDOW_ROWS;
        this.frontendRenderedCount = 0;
        this.filteredListJson = [];
    ...
    init() {
        ...
        try {
            ...
            var pageVars = JSON.parse(entityPageVarsData);
            if (typeof pageVars.PageSize != "undefined")
            ...
            this.listJson = entityPageLoadData.result.data?.ListData ? entityPageLoadData.result.data.ListData : [];
            this.fields = this.properties.FIELDS || "All";
            this.keyField = this.properties.KEYFIELD || "";

            if (this.pageSize == 0 || this.pageSize > this.listJson.length)
            ...
            if (this.fields == "All")
                this.modificationFields = "modifiedon,modifiedby,createdon,createdby,cancel,cancelremarks";
            else {
                const dynamicFields = ['modifiedby', 'modifiedon', 'createdby', 'createdon' ,'cancel', 'cancelremarks'];
            ...
            // Initialize row count manager
            rowsCountHelper = new RowsCountHelper();
            rowsCountHelper.init();
    ...
    generateHTMLBasedOnDataType(fldProps, rowData) {
        ...
        var displayValue = fldtype.toUpperCase() === "CURRENCY"
            ? formatNumberBasedOnMillions(fldValue, fldProps)
            : fldValue;

        // Skip "Modified On" and "Modified By" fields
        if (fldkey.toLowerCase() === 'modifiedon' || fldkey.toLowerCase() === 'username' || fldkey.toLowerCase() === 'createdby' || fldkey.toLowerCase() === 'createdon' || fldkey.toLowerCase() === 'cancel' || fldkey.toLowerCase() === 'cancelremarks') {
            return '';
        }
        ...
        if (_this.captionsEnabled) {
            html = `<div class="Data-fields-items" ${tooltipAttr}>
                        <div class="caption-style">
                            ...
                            <span class="txt-bold Data-field-value">${displayValue}</span>
                        </div>
                    </div>`;
        }
        ...
        switch (fldtype.toUpperCase()) {
            ...
            case 'CURRENCY':
                html = html || `<div class="Data-fields-items Department-field" ${tooltipAttr} data-name="${fldcap?.toLowerCase().replaceAll("*", "").replaceAll(" ", "") ?? ""}">
                            <span class="material-icons material-icons-style material-icons-2">payments</span><span class="txt-bold Data-field-value">${displayValue}</span>
                        </div>`;
                break;
            case 'DATE':
    ...
    updateStreamingWindow(fetchedRows, requestedPageNo) {
        if (!this.streamingAllRecords) {
            this.listJson.push(...fetchedRows);
            this.visibleRowRangeStart = this.listJson.length > 0 ? 1 : 0;
            this.visibleRowRangeEnd = this.listJson.length;
            return;
        }

        // In "show all" mode, API returns full dataset once (pageSize = 0).
        if (requestedPageNo !== 1) return;

        this.allRowsJson = Array.isArray(fetchedRows) ? fetchedRows : [];
        this.frontendRenderedCount = Math.min(this.frontendChunkSize, this.allRowsJson.length);
        this.listJson = this.allRowsJson.slice(0, this.frontendRenderedCount);
        this.allRecordsLoaded = this.frontendRenderedCount >= this.allRowsJson.length;

        if (this.frontendRenderedCount === 0) {
        ...
        this.visibleRowRangeStart = 1;
        this.visibleRowRangeEnd = this.frontendRenderedCount;
    }

    renderCurrentDataWindow(scrollDirection) {
        const activeSearchTerm = (this.currentSearchTerm || "").trim().toLowerCase();
        if (activeSearchTerm !== "") {
            this.filteredListJson = this.listJson.filter((rowData) =>
                Object.values(rowData).some(value =>
                    String(value ?? "").toLowerCase().includes(activeSearchTerm)
                )
            );
        }
        else {
            this.filteredListJson = this.listJson.slice();
        }

        createTableViewHTML(this.filteredListJson, 1);
        bindTableViewScroll();
    ...
    loadNextFrontendChunk() {
        if (!this.streamingAllRecords || !Array.isArray(this.allRowsJson) || this.allRowsJson.length === 0) {
            return false;
        }

        const currentWindowSize = this.visibleRowRangeEnd - this.visibleRowRangeStart + 1;
        if (this.visibleRowRangeEnd >= this.allRowsJson.length && currentWindowSize >= this.frontendChunkSize) {
            this.allRecordsLoaded = true;
            return false;
        }
        ...
        if ((nextEnd - nextStart) < this.frontendWindowMaxRows) {
            // Grow from 100 to 200 before shifting window.
            nextEnd = Math.min(nextEnd + this.frontendChunkSize, this.allRowsJson.length);
        } else {
            // Keep max window 200 by removing top 100 and adding next 100.
            nextStart = Math.min(nextStart + this.frontendChunkSize, this.allRowsJson.length);
            nextEnd = Math.min(nextEnd + this.frontendChunkSize, this.allRowsJson.length);
        }
        ...
        this.listJson = this.allRowsJson.slice(nextStart, nextEnd);
        this.frontendRenderedCount = nextEnd;
        this.allRecordsLoaded = this.visibleRowRangeEnd >= this.allRowsJson.length;

        this.renderCurrentDataWindow("down");
        return true;
    }
    ...
    loadPreviousFrontendChunk() {
        ...
        if (this.visibleRowRangeStart <= 1) {
            return false;
        }

        let nextStart = Math.max((this.visibleRowRangeStart - 1) - this.frontendChunkSize, 0);
        const targetWindow = Math.min(this.frontendWindowMaxRows, this.allRowsJson.length);
        let nextEnd = Math.min(nextStart + targetWindow, this.allRowsJson.length);
        ...
        this.renderCurrentDataWindow("up");
        return true;
    }

    getVisibleRowRange() {
        if (!Array.isArray(this.listJson) || this.listJson.length === 0) {
            return { start: 0, end: 0 };
        }

        if (this.streamingAllRecords) {
            return {
                start: this.visibleRowRangeStart || 1,
                end: this.visibleRowRangeEnd || this.listJson.length
            };
        }

        return { start: 1, end: this.listJson.length };
    }
    ...
    getEntityListData(requestedPageNo) {
        ShowHideDimmer(true);
        let _this = this;
        ...
        // In "show all" mode we fetch once (page 1, pageSize 0) and then paginate on frontend.
        if (_this.streamingAllRecords && requestedPageNo > 1) {
            ShowHideDimmer(false);
            return;
        }

        let data = { transId: _this.entityTransId, fields: _this.fields, pageNo: requestedPageNo, pageSize: _this.pageSize, filter: _this.filter };
        ...
        this.callAPI(url, data, true, result => {            
            if (result.success) {
                try {
                    let listResponse = JSON.parse(JSON.parse(result.response).d);
                    let listJson = listResponse?.result?.data?.ListData;

                    if (listJson.length === 0) {
                        if (requestedPageNo === 1) {
                            _this.listJson = [];
                            _this.allRowsJson = [];
                            _this.frontendRenderedCount = 0;
                            _this.visibleRowRangeStart = 0;
                            _this.visibleRowRangeEnd = 0;
                            ...
                            if (_this.pageSize.toString() == "0" || _this.streamingAllRecords)
                                _this.allRecordsLoaded = true;
                        ...
                        _this.updateStreamingWindow(listJson, requestedPageNo);

                        _this.maxPageNumber = requestedPageNo;

                        if (_this.streamingAllRecords) {
                            _this.renderCurrentDataWindow();
                        }
                        else {
                            createTableViewHTML(_this.listJson, requestedPageNo);
                            bindTableViewScroll();
                            $('[data-toggle="tooltip"]').tooltip();
                            initReadMore();
                        }
                ...
                try {
                    ...
                    if (condition == "General") {
                        if (chartsData.length > 0) {
                            ...
                            const total = chartsData && chartsData[0] ? chartsData[0].totrec : 0;
                            rowsCountHelper.setTotalRecords(total);
                            return;
                        }
                    } else {
    ...
    editEntity(recordId, rowNo) {
        ...
        if (rowNo == "1")
            pageType = "first";
        else if (rowNo == this.listJson.length.toString())
            pageType = "last";
        else
            pageType = "middle";
...
function createFieldsLayout() {
    ...
    let skipFields = ["transid", "modifiedby", "modifiedon", "createdby", "createdon", "username", "axpeg_nextlevel", "cancel", "cancelremarks"];
        ...
        <div class="KC_Items_Content">
            <table class="table table-hover">
                <tbody>
                    ...
                    <tr><td><input type="checkbox" id="chk_cancel" class="chk-fields chk-modification" value="cancel"></td><td><label for="chk_cancel">Cancelled?</label></td></tr>
                    <tr><td><input type="checkbox" id="chk_cancelremarks" class="chk-fields chk-modification" value="cancelremarks"></td><td><label for="chk_cancelremarks">Cancel Remarks</label></td></tr>
                </tbody>
            </table>
        </div>
    </div>`;
...
function updateSelectOptions() {
    ...
    const dynamicFields = ['modifiedby', 'modifiedon', 'createdby', 'createdon', 'cancel', 'cancelremarks'];
    checkFields.forEach((checkbox) => {
        if (checkbox.checked && !(dynamicFields.indexOf(checkbox.value) > -1)) {
            const fieldName = checkbox.dataset.fldcap ?? "";
            const option = document.createElement("option");
...
if (searchBoxButton) {
    searchBoxButton.addEventListener("click", function () {
        // onclick toggles visibility first; then focus if search input is shown.
        setTimeout(() => {
...
function liveSearch() {
    ...
    _entity.currentSearchTerm = searchTerm;

    const sourceJson = (searchTerm !== "" && _entity.streamingAllRecords && Array.isArray(_entity.allRowsJson) && _entity.allRowsJson.length > 0)
        ? _entity.allRowsJson
        : (Array.isArray(_entity.listJson) ? _entity.listJson : []);
    _entity.filteredListJson = searchTerm === ""
        ? sourceJson.slice()
    ...
    if (_entity.filteredListJson.length === 0) {
        createTableViewHTML(sourceJson, 1);
        bindTableViewScroll();
        $('[data-toggle="tooltip"]').tooltip();
    ...
    createTableViewHTML(_entity.filteredListJson, 1);
    bindTableViewScroll();
    $('[data-toggle="tooltip"]').tooltip();
    if (rowsCountHelper) {
        rowsCountHelper.init();
...
function formatFieldName(field) {
    return field
        ...
        .replace(/cancelremarks/g, 'Cancel Remarks')
        .replace(/cancel/g, 'Cancelled?')        
        .replace(/^ /, '');
}
...
function createTableViewHTML(listJson, _pageNo) {
    ...
    const filteredMetaData = _entity.metaData.filter(item => item.listingfld === "T" && item.hide != "T");
    ...
    if (typeof _entity.modificationFields === 'string') {
    ...
    listJson.forEach((rowData, index) => {
        ...
        filteredMetaData.forEach(field => {
            if (field.fldname.toLowerCase() !== keyCol.toLowerCase() && fieldsWithData.has(field.fldname.toLowerCase()) && !excludedFields.has(field.fldname) && (!hideTransid || field.fldname.toLowerCase() !== 'transid')) {
                ...
                if (field.cdatatype === 'Check box' || field.fdatatype === 'Check box' || field.fldname === 'cancel') {
                    cellContent = `
                    ...
                    cellContent = formatNumberBasedOnMillions(fieldValue, field);
...
function handleExport(action, tableSelector) {
    ...
    // Always clean up hidden export table/dataTable instances to avoid memory growth.
    ...
    const dataCount = (_entity.streamingAllRecords && Array.isArray(_entity.allRowsJson) && _entity.allRowsJson.length > 0)
        ? _entity.allRowsJson.length
        : _entity.listJson.length;
    const transId = _entity.entityTransId;
    ...
    const useQueueExport = action.toUpperCase() !== 'PRINT';
...
function createHiddenTableFromMetadata() {
    ...
    const exportRows = (_entity.streamingAllRecords && Array.isArray(_entity.allRowsJson) && _entity.allRowsJson.length > 0)
        ? _entity.allRowsJson
        : _entity.listJson;
...
function applyTableScrollLayout(tableDiv) {
    if (!tableDiv) return;

    const tableTop = tableDiv.getBoundingClientRect().top;
    const availableHeight = Math.max(260, window.innerHeight - tableTop - 24);
    tableDiv.style.height = `${availableHeight}px`;
    tableDiv.style.maxHeight = `${availableHeight}px`;
    tableDiv.style.boxSizing = 'border-box';
    tableDiv.style.paddingRight = '0';
    tableDiv.style.width = '100%';
    tableDiv.style.overflowY = 'auto';
    tableDiv.style.overflowX = 'auto';
    tableDiv.style.scrollbarGutter = '';
}

function bindTableViewScroll() {
    const tableDiv = document.querySelector('#table-body_Container .table-responsive');
    if (!tableDiv) return;

    // Keep table scroll visible and stable across zoom levels.
    applyTableScrollLayout(tableDiv);

    tableDiv.removeEventListener('scroll', scrollListener);
    tableDiv.addEventListener('scroll', scrollListener);

    if (!tableScrollResizeBound) {
        window.addEventListener('resize', function () {
            const activeTableDiv = document.querySelector('#table-body_Container .table-responsive');
            applyTableScrollLayout(activeTableDiv);
        });
        tableScrollResizeBound = true;
    }
}
...
function scrollListener(event) {
    ...
    if (_entity.streamingAllRecords && (_entity.currentSearchTerm || "").trim() !== "") {
    ...
    if (this.lastScrollTop !== currentScrollTop) {
        ...
        if (!isFetching) {
            if (isScrollAtBottomWithinDiv(myDiv)) {
                ...
                if (_entity.streamingAllRecords) {
                    ShowHideDimmer(true);
                    setTimeout(() => {
                        try {
                            _entity.loadNextFrontendChunk();
                        } finally {
                            ShowHideDimmer(false);
                            isFetching = false;
                        }
            ...
            else if (_entity.streamingAllRecords && isScrollAtTopWithinDiv(myDiv)) {
                isFetching = true;
                ShowHideDimmer(true);
                setTimeout(() => {
                    try {
                        _entity.loadPreviousFrontendChunk();
                    } finally {
                        ShowHideDimmer(false);
                        isFetching = false;
                    }
...
function formatNumberBasedOnMillions(amount, fieldMeta) {
    return formatCurrencyAmount(amount, fieldMeta, shouldUseMillionGrouping());
}

function shouldUseMillionGrouping() {
    return ((Object.values(window.top.allGloblVars.globalVars)
        .map((gv) => gv)
...
function formatCurrencyAmount(amount, fieldMeta, useMillionGrouping) {
    const normalized = normalizeCurrencyValue(amount);
    if (!normalized) return '';

    const decimalPlaces = resolveCurrencyDecimalPlaces(fieldMeta, normalized.fractionPart);
    let { integerPart, fractionPart, isNegative, numericValue } = normalized;

    if (decimalPlaces !== fractionPart.length) {
        const fixedValue = numericValue.toFixed(decimalPlaces);
        const [fixedIntegerPart, fixedFractionPart = ""] = fixedValue.split(".");
        integerPart = fixedIntegerPart.replace("-", "");
        fractionPart = fixedFractionPart;
        isNegative = fixedValue.startsWith("-");
    }

    const groupedInteger = useMillionGrouping
        ? groupByThousands(integerPart)
        : groupByIndianDigits(integerPart);

    return `${isNegative ? "-" : ""}${groupedInteger}${fractionPart ? `.${fractionPart}` : ""}`;
}

function normalizeCurrencyValue(amount) {
    if (amount === null || typeof amount === "undefined") return null;

    const rawValue = amount.toString().trim().replace(/,/g, "");
    if (rawValue === "" || !/^[+-]?(?:\d+\.?\d*|\.\d+)$/.test(rawValue)) return null;

    const isNegative = rawValue.startsWith("-");
    const unsignedValue = rawValue.replace(/^[+-]/, "");
    const [integerDigits = "0", fractionDigits = ""] = unsignedValue.split(".");
    const integerPart = integerDigits.replace(/^0+(?=\d)/, "") || "0";
    const numericValue = Number(`${isNegative ? "-" : ""}${integerPart}${fractionDigits ? `.${fractionDigits}` : ""}`);
...
function resolveCurrencyDecimalPlaces(fieldMeta, fractionPart) {
    const explicitDecimalPlaces = extractCurrencyDecimalPlaces(fieldMeta);
    if (explicitDecimalPlaces !== null) {
        return explicitDecimalPlaces;
    }

    return fractionPart.length;
}

function extractCurrencyDecimalPlaces(fieldMeta) {
    ...
    for (const key of metadataKeys) {
        const parsedValue = parseDecimalCount(fieldMeta[key]);
        if (parsedValue !== null) {
            return parsedValue;
        }
    }

    return extractDecimalPlacesFromProps(fieldMeta.props);
}
...
function extractDecimalPlacesFromProps(props) {
    if (!props) return null;

    const propString = typeof props === "string" ? props : JSON.stringify(props);
    const decimalMatch = propString.match(/(?:decimalplaces|decimals|decimaldigits|noofdecimals|numdecimals|fdecimal|flddecimal)[=:\s~"]+(\d+)/i);

    return decimalMatch ? parseDecimalCount(decimalMatch[1]) : null;
}

function InsertCommas(amount, fieldMeta) {
    return formatCurrencyAmount(amount, fieldMeta, true);
}
...
function CommaFormatted(amount) {
    ...
    const groupedInteger = groupByThousands(normalized.integerPart);
    return `${normalized.isNegative ? "-" : ""}${groupedInteger}${normalized.fractionPart ? `.${normalized.fractionPart}` : ""}`;
}

function formatIndianNumber(amount, fieldMeta) {
    return formatCurrencyAmount(amount, fieldMeta, false);
}

function groupByThousands(integerPart) {
    return integerPart.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
}

function groupByIndianDigits(integerPart) {
    if (integerPart.length <= 3) {
        return integerPart;
    }

    const lastThreeDigits = integerPart.slice(-3);
    const remainingDigits = integerPart.slice(0, -3).replace(/\B(?=(\d{2})+(?!\d))/g, ",");
...
class RowsCountHelper {
    constructor() {
        this.totalRecords = null;
        this.rowCountElement = null;
        this.rowsCountIconElement = null;
        this.loadAllRowsIconElement = null;
        this.containerElement = null;
        this.userClickedQuestionMark = false;
    }

    init() {
        // Remove existing if any
        const existing = document.querySelector('.global-rowcount-container');
        if (existing) existing.remove();

        if (!_entity.streamingAllRecords && !this.totalFetched && (_entity.listJson.length < ((_entity.pageSize * _entity.maxPageNumber) - 1)))
            this.totalRecords = _entity.listJson.length;

        if (_entity.streamingAllRecords && Array.isArray(_entity.allRowsJson) && _entity.allRowsJson.length > 0) {
            this.totalRecords = _entity.allRowsJson.length;
        }

        const visibleRange = _entity.getVisibleRowRange();
        const visibleRangeStart = visibleRange.start > 0 ? visibleRange.start : 1;
        const visibleRangeEnd = visibleRange.end > 0 ? visibleRange.end : (_entity.pageSize * _entity.maxPageNumber);
        
        var rowsCountLoaded = typeof this.totalRecords != "undefined" && this.totalRecords > 0;

        if (_entity.allRecordsLoaded) {
            rowsCountLoaded = true;
            this.totalRecords = visibleRange.end;
        }        

        // Create container with better alignment
        this.containerElement = document.createElement('div');
        this.containerElement.className = 'global-rowcount-container';
        this.containerElement.style.cssText = `display: flex; align-items: center; gap: 8px; padding: 4px 12px; `;

        // Add text element
        this.rowCountElement = document.createElement('span');
        this.rowCountElement.id = 'global-rowcount-text';
        this.rowCountElement.style.cssText = 'font-weight: 600; white-space: nowrap;';
        const activeSearchTerm = (_entity.currentSearchTerm || "").trim();
        const filteredCount = Array.isArray(_entity.filteredListJson) ? _entity.filteredListJson.length : 0;

        if (rowsCountLoaded) {
            if (_entity.streamingAllRecords && this.totalRecords > 0) {
                if (activeSearchTerm !== "") {
                    const start = filteredCount > 0 ? 1 : 0;
                    this.rowCountElement.textContent = `Rows: ${start}-${filteredCount} of ${this.totalRecords}`;
                } else {
                    this.rowCountElement.textContent = `Rows: 1-${this.totalRecords} of ${this.totalRecords}`;
                }
            } else {
                this.rowCountElement.textContent = `Rows: ${visibleRangeStart}-${visibleRangeEnd} of ${this.totalRecords}`;
            }
            this.showQuestionMark(false);
        }
        else {
            this.rowCountElement.textContent = `Rows: ${visibleRangeStart}-${visibleRangeEnd} of`;

            // Add icon element
            this.rowsCountIconElement = document.createElement('span');
            this.rowsCountIconElement.id = 'global-rowcount-icon';
            this.rowsCountIconElement.className = 'material-icons material-icons-style material-icons-3';
            this.rowsCountIconElement.style.cssText = 'cursor: pointer; font-weight: 600; color: #666; display: inline-block; vertical-align: middle; font-size: 16px;';
            this.rowsCountIconElement.textContent = 'question_mark';
            this.rowsCountIconElement.title = 'Click to view total records';

            // Add click handler
            this.rowsCountIconElement.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopImmediatePropagation();
                this.handleTotalCountClick();
                return false;
            });
        ...
        if (!_entity.allRecordsLoaded && !_entity.streamingAllRecords) {
            // Add icon element
            this.loadAllRowsIconElement = document.createElement('span');
            this.loadAllRowsIconElement.id = 'global-rowcount-icon';
            this.loadAllRowsIconElement.className = 'material-icons material-icons-style material-icons-3';
            this.loadAllRowsIconElement.style.cssText = 'cursor: pointer; font-weight: 600; color: #666; display: inline-block; vertical-align: middle; font-size: 16px;';
            this.loadAllRowsIconElement.textContent = 'play_for_work';
            this.loadAllRowsIconElement.title = 'Show all records';

            // Add click handler
            this.loadAllRowsIconElement.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopImmediatePropagation();
                this.handleFetchAllRowsClick();
                return false;
            });

            this.showLoadAllRecords(true);
        }
        else {
            this.showLoadAllRecords(false);
        }

        this.containerElement.appendChild(this.rowCountElement);
        if (this.rowsCountIconElement) this.containerElement.appendChild(this.rowsCountIconElement);
        if (this.loadAllRowsIconElement) this.containerElement.appendChild(this.loadAllRowsIconElement);


        if (!this.containerElement) return;

        // First remove from any parent
        ...
        let rowCountToolBar;
        if (_entity.properties.RIGHTPANEL == "expanded")
            rowCountToolBar = document.querySelector("#Entity_management_Right .card-toolbar");
        else
            rowCountToolBar = document.querySelector("#Entity_management_Left .card-toolbar");

        if (rowCountToolBar && typeof rowCountToolBar != "undefined" && _entity.listJson.length > 0) {
            rowCountToolBar.appendChild(this.containerElement);
        }
    ...
    showQuestionMark(show) {
        if (this.rowsCountIconElement) {
            if (show) {
                this.rowsCountIconElement.className = 'material-icons material-icons-style material-icons-3';
                this.rowsCountIconElement.style.cssText = `cursor: pointer; font-weight: 600; color: #666; display: inline-block; vertical-align: middle; font-size: 16px;`;
                this.rowsCountIconElement.textContent = 'question_mark';
                ...
                this.rowsCountIconElement.style.cssText = `display: none;`;
            }
        }
    }

    showLoadAllRecords(show) {
        if (this.loadAllRowsIconElement) {
            if (show) {
                this.loadAllRowsIconElement.className = 'material-icons material-icons-style material-icons-3';
                this.loadAllRowsIconElement.style.cssText = `cursor: pointer; font-weight: 600; color: #666; display: inline-block; vertical-align: middle; font-size: 16px;`;
                this.loadAllRowsIconElement.textContent = 'play_for_work';
                ...
                this.loadAllRowsIconElement.style.cssText = `display: none;`;
            }
        }
    }
    ...
    handleTotalCountClick() {
        if (this.userClickedQuestionMark) return;

        this.userClickedQuestionMark = true;
        this.rowsCountIconElement.textContent = '…';
        this.rowsCountIconElement.style.cursor = 'wait';

        this.fetchTotalCount();
    }

    handleFetchAllRowsClick() {       
        if (_entity.allRecordsLoaded || _entity.streamingAllRecords) return;        

        this.loadAllRowsIconElement.textContent = '…';
        this.loadAllRowsIconElement.style.cursor = 'wait';

        this.loadAllRecords();
    }

    fetchTotalCount() {
        if (this.totalFetched) return; // Already fetched

        const payload = {
            aggField: "count",
            aggFunc: "count",
            aggTransId: _entity.entityTransId,
            groupField: "all",
            groupTransId: _entity.entityTransId,
            page: "Entity",
            transId: _entity.entityTransId
        };

        const base = top.window.location.href.toLowerCase()
            .substring(0, top.window.location.href.toLowerCase().indexOf("/aspx/"));
        const url = base + '/aspx/Analytics.aspx/GetAnalyticsChartsDataWS';
        ...
        $.ajax({
            ...
            success: (resp) => {
                try {
                    const parsed = JSON.parse(resp.d);
                    const arr = JSON.parse(parsed.result.charts[0].data_json);
                    const total = arr && arr[0] ? arr[0].totrec : 0;
                    this.setTotalRecords(total);
                } catch (ex) {
                    console.error('fetchTotalCount: parse total error', ex);
                    this.setTotalRecords(null);
                }
            },
            error: (err) => {
                console.error('fetchTotalCount: fetch total error', err);
                this.setTotalRecords(null);
            }
        });
    }
    ...
    loadAllRecords() {
        _entity.enableStreamingAllRecordsMode();
        pageNo = 1;
        _entity.getEntityListData(1);
    }
}
