...
public partial class aspx_htmlPages : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else if (htmlTemplateName != string.Empty)
        {
            ...
            {
                axpertWebUrl = "";
                path = axpertWebUrl + projectName + "/LandingPageHTMLTemplates/" + htmlTemplateName + "?v=" + DateTime.Now.ToString("ddMMyyyyHHmmss");
                FileInfo _htmlFile = new FileInfo(HttpContext.Current.Server.MapPath("~/" + projectName + "/LandingPageHTMLTemplates/" + htmlTemplateName));
                if (_htmlFile.Exists)
                {
                    if (path != string.Empty)
                    {
                        ...
                        {
                            errorLog = logobj.CreateLog("CallGetFileNames - Call Get HTML File loading exception for available file => " + ex.Message, sessionId, "LandingPageHTMLTemplates", "", "true");
                            Response.Redirect("err.aspx?errmsg=Exception while loading this page.");
                        }
                    }
                }
                ...
                {
                    errorLog = logobj.CreateLog("CallGetFileNames - Call Get HTML File is not found in the given path => " + path, sessionId, "LandingPageHTMLTemplates", "", "true");
                    Response.Redirect("err.aspx?errmsg=File doesn't exist in the path.");
                }
            }
            catch (Exception ex) { }
